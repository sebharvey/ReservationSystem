...
namespace Res.Api.Order.Application.Services
{
    public class OrderService : IOrderService
    {
        ...
        private List<OrderRetrieveResponse.BoardingPass> GetValidBoardingPasses(Pnr pnr)
        {
            var boardingPasses = new List<OrderRetrieveResponse.BoardingPass>();
            ...
            foreach (var segment in pnr.Data.Segments.Where(s => !s.IsSurfaceSegment))
            {
                ...
                var validPassengers = pnr.Data.Tickets
                    .Where(ticket => ticket.Coupons.Any(coupon =>
                        ...
                        coupon.Status == OrderRetrieveResponse.CouponStatus.CheckedIn))
                    .Select(ticket => new
                    {
                        Ticket = ticket,
